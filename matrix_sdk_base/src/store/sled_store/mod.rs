...
pub fn decode_batch_idx_position(key: &[u8]) -> Option<(usize, usize)> {
    let (first, second) = key.split_at(std::mem::size_of::<usize>());
...
impl SledStore {
                ...
                |(
                    ...
                    for (room, timeline) in &changes.timeline {
                        ...
                        let expandable = {
                            ...
                            {
                                // Check if next event in store fits next event
                            ...
                            let found_last = if let Some((batch_idx, current_position)) =
                                event_id_to_position
                                    ...
                                    .map(|e| decode_batch_idx_position(&e).unwrap())
                            {
                                let current_position = current_position - timeline.events.len();
                        ...
                        for event in &timeline.events {
                            if event_id_to_position
                                .get((room.as_str(), event.event_id().as_str()).encode())
                                .transpose()
                                .is_none()
                            {
                                events.push(event);
                        ...
                        let current_position = if new_start_token && !new_end_token {
                            current_position - duplicated
                        } else if new_end_token && !new_start_token {
                            current_position + duplicated
                        } else {
                            current_position
                        };
                        ...
                        for (position, event) in events.iter().enumerate() {
                            ...
                            {
                                if let Some((batch_idx, position)) = event_id_to_position
                                    ...
                                    .map(|t| decode_batch_idx_position(&t).unwrap())
                                {
                                    if let Some(full_event) = timeline_events
                                        .get((room.as_str(), batch_idx, position).encode())?
                                        .map(|t| {
                        ...
                        if new_end_token {
                            if let Some(end) = &timeline.end {
                                ...
                                end_token_to_batch_idx_position.insert(
                                    ...
                                    (batch_idx, current_position + events.len()).encode(),
                                )?;
                            }
                        }
                    }
    ...
    async fn get_timeline(
        ...
        let (mut batch_idx, mut position) = beginning;
