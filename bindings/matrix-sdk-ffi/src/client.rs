...
impl Client {
    ...
    /// Get a preview of the given URL, as generated by the homeserver.
    ///
    /// Returns `None` if the homeserver has no preview to offer for this URL.
    ///
    /// The preview is returned as a raw JSON string rather than a typed
    /// structure. The response follows the OpenGraph specification, whose set
    /// of fields is open-ended, and additionally carries Matrix's own
    /// `matrix:image:size`; decoding into a fixed structure here would
    /// silently drop any field this binding did not anticipate. Clients are
    /// expected to decode whichever fields they support.
    ///
    /// # Arguments
    ///
    ...
    ///   timestamp in milliseconds. Note that the homeserver may return a
    ///   preview for a different point in time, and that this is deprecated
    ///   since Matrix 1.11 — most callers should pass `None`.
    pub async fn get_media_preview(
        &self,
        url: String,
        ts: Option<u64>,
    ) -> Result<Option<String>, ClientError> {
        // `new_saturating` rather than `new(..).unwrap()`: this value comes
        // straight from a foreign-language caller, and a nonsensical timestamp
        // must not panic across the FFI boundary.
        let ts = ts.map(|ts| MilliSecondsSinceUnixEpoch(UInt::new_saturating(ts)));

        debug!(url, "requesting URL preview");
        Ok(self
            .inner
            .media()
            .get_media_preview(&url, ts)
            .await?
            // `.get()` hands back the original JSON text, so this avoids a
