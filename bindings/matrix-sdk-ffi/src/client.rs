...
/// A listener for key upload errors triggered by requests to /keys/upload.
#[matrix_sdk_ffi_macros::export(callback_interface)]
pub trait KeyUploadErrorListener: SyncOutsideWasm + SendOutsideWasm {
    /// Called once when uploading keys fails.
    fn on_key_upload_error(&self);
}
...
impl Client {
    ...
    /// Subscribe to key upload errors triggered by requests to /keys/upload.
    pub fn subscribe_to_key_upload_errors(
        &self,
        listener: Box<dyn KeyUploadErrorListener>,
    ) -> Arc<TaskHandle> {
        let mut subscriber = self.inner.subscribe_to_key_upload_errors();

        Arc::new(TaskHandle::new(get_runtime_handle().spawn(async move {
            loop {
                match subscriber.recv().await {
                    Ok(_) => listener.on_key_upload_error(),
                    Err(err) => {
