...
impl SqliteSessionStoreBuilder {
    /// Construct a SqliteSessionStoreBuilder and set the paths that the client
...
impl SessionStoreConfig {
    pub(crate) fn build(&self) -> Result<SessionStoreResult, ClientBuildError> {
        match self {
            #[cfg(feature = "sqlite")]
            SessionStoreConfig::Sqlite(config) => {
                let data_path = Path::new(&config.paths.data_path);
                ...
                sqlite_store_config = sqlite_store_config.passphrase(config.passphrase.as_deref());
            ...
            #[cfg(feature = "indexeddb")]
            SessionStoreConfig::IndexedDb(config) => Ok(SessionStoreResult::IndexedDb {
