...
//! ## Laziness
//!
//! [`LatestEvents`] is lazy, it means that, despites [`LatestEvents`] is
...
//! [`LatestEvents::for_room`] is called for a particular room, no latest event
//! will ever be computed for that room (and similarly with
...
pub use latest_event::LatestEvent;
...
/// The entry point to fetch the [`LatestEvent`] for rooms or threads.
#[derive(Clone, Debug)]
...
impl LatestEvents {
    ...
    ///
    /// Note that a read lock guard is returned if the [`LatestEvent`] exists.
    /// One doesn't want to keep this lock alive for too long, otherwise the
    /// whole [`LatestEvents`] API is blocked. Use [`LatestEvent::get`] or
    /// [`LatestEvent::subscribe`] as quick as possible, and then drop this
    /// lock guard.
    pub async fn for_room(&self, room_id: &RoomId) -> Option<RwLockReadGuard<'_, LatestEvent>> {
        self.state.registered_rooms.for_room(room_id).await
    }
...
struct RegisteredRooms {
    ...
    ///
    /// When a room is registered (with [`LatestEvents::for_room`] or
    /// [`LatestEvents::for_thread`]) or unregistered (with
...
mod tests {
    ...
    async fn test_latest_events_are_lazy() {
        ...
        // Now let's fetch two rooms.
        let _ = latest_events.for_room(room_id_0).await.unwrap();
        let _ = latest_events.for_room(room_id_1).await.unwrap();
        ...
        // Now let's fetch one thread for two room.
        let _ = latest_events.for_thread(room_id_1, thread_id_1_0).await.unwrap();
        let _ = latest_events.for_thread(room_id_2, thread_id_2_0).await.unwrap();
    ...
    async fn test_forget_room() {
        ...
        let _ = latest_events.forget_room(room_id_0).await;
    ...
    async fn test_forget_thread() {
        ...
        let _ = latest_events.for_thread(room_id_0, thread_id_0_0).await.unwrap();
        ...
        let _ = latest_events.forget_thread(room_id_0, thread_id_0_0).await;
