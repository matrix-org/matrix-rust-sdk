...
impl Channel {
    ...
    pub(super) async fn send(&mut self, message: String) -> Result<(), HttpError> {
        ...
        // We successfully send out a message, get the sequence_token and update
        // our internal copy of the sequence_token.
    ...
    pub(super) async fn receive(&mut self) -> Result<String, HttpError> {
        loop {
            ...
            // if sequence token is the same as our current one, it means no new
            // message and map into not modified HTTP status
    ...
    async fn receive_single_message(&mut self) -> Result<RendezvousMessage, HttpError> {
        ...
        // since the rendezvous API has changed it doesn't make sense to use the
        // http status code at this layer
        ...
        // We received a new sequence_token, put it into the copy of our
        // sequence_token.
        self.sequence_token = response.sequence_token;
