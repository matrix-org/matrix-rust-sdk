...
pub trait MediaFetcher: SendOutsideWasm + SyncOutsideWasm + fmt::Debug {
    ...
    /// Fetches the media content for the given [`MediaRequestParameters`],
    /// using the provided [RequestConfig] and reporting download progress
    /// through `progress`. Returns either a byte array or an
    /// [`crate::Error`].
...
impl MediaFetcher for DefaultMediaFetcher {
    fn fetch_media_content<'a>(
        ...
        let request_config = client
            .request_config()
            // Downloading a file should have no timeout as we don't know the
            // network connectivity available for the user or the file size
            .timeout(Some(Duration::MAX));

        self.fetch_media_content_with_config(client, request, request_config, progress)
    }
