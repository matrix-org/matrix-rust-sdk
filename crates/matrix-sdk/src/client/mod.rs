...
impl Client {
    ...
    pub async fn create_room(
        ...
        // If the room is already in our store return it.
        if let Some(room_from_store) = self.get_joined_room(&room_id) {
            return Ok(room_from_store)
        }
        // otherwise wait for a SyncStateEvent which will give us the room.
