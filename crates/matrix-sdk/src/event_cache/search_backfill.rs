...
use super::{
    ...
    back_pagination_queue::{
        BATCH_SIZE, BackPaginationQueue, BackPaginationRequest, BackPaginationStopReason, Priority,
    },
...
impl EventCache {
    ...
    pub async fn run_search_backfill(&self, strategy: BackPaginationStrategy) {
        ...
        let mut rooms = rooms_by_relevancy(&client);
        ...
        for week in 1..=MAX_BACKFILL_WEEKS {
            ...
            for chunk in rooms.chunks(ROOM_BATCH) {
                ...
                for room_id in chunk {
                    ...
                    match enqueue(&queue, room_id.clone(), max_age) {
                        Ok(handle) => handles.push((room_id.clone(), handle)),
                        Err(err) => {
                            warn!(%room_id, "couldn't enqueue a search backfill request: {err}")
                        }
                    }
                }
                ...
                for (room_id, handle) in handles {
                    ...
                    if reason == BackPaginationStopReason::ReachedTimelineStart {
...
fn rooms_by_relevancy(client: &Client) -> Vec<OwnedRoomId> {
...
fn enqueue(
    ...
    super::back_pagination_queue::BackPaginationQueueError,
> {
