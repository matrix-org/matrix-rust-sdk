...
use crate::{
    ...
    room::{IncludeRelations, PushContext, RelationsOptions, WeakRoom},
    Room,
};
        ...
        loop {
            match outcome {
                LoadMoreEventsBackwardsOutcome::Gap { prev_token } => {
                    ...
                    let push_context =
                        RoomEventCacheInner::push_context_for_threads_subscriptions(&room).await;
                    ...
                    if let Some((outcome, thread_subscriptions)) = state
                        ...
                        .await
                    {
                        self.inner.subscribe_to_new_threads(thread_subscriptions).await;
...
pub(super) struct AutomaticThreadSubscriptions(pub BTreeMap<OwnedEventId, OwnedEventId>);
...
impl RoomEventCacheInner {
    ...
    pub(super) async fn subscribe_to_new_threads(
        &self,
        new_thread_subs: AutomaticThreadSubscriptions,
    ) {
        if new_thread_subs.0.is_empty() {
            return;
        }
        ...
        // If the client has enabled thread subscriptions, we don't have anything to do.
    ...
    /// support for those.
    pub(super) async fn push_context_for_threads_subscriptions(room: &Room) -> Option<PushContext> {
        if !room.client().enabled_thread_subscriptions() {
            return None;
        }
        ...
        // would trigger a mention. Of course, we're not interested in an
        // in-thread event causing a mention, because it's part of a thread
        // we've subscribed to. As a result, make the `PushContext` evaluation
        // of thread subscriptions a no-op, so only the other rules are
        // evaluated.
        let push_room_condition_ctx = ctx.push_condition_room_ctx_mut().clone();
        *ctx.push_condition_room_ctx_mut() = push_room_condition_ctx
            .with_has_thread_subscription_fn(|_event_id| Box::pin(async move { false }));
    ...
    async fn handle_timeline(
        ...
        let push_context = if let Some(room) = self.weak_room.get() {
            Self::push_context_for_threads_subscriptions(&room).await
        } else {
            None
        };
...
mod private {
    ...
    use crate::{
        event_cache::{
            ...
            room::{
                threads::{should_subscribe_thread, ThreadEventCache},
                AutomaticThreadSubscriptions,
            },
        ...
        async fn update_threads(
            ...
            for (thread_root, new_events) in new_events_by_thread {
                ...
                if let Some(push_context) = &push_context {
                    for ev in new_events.iter().rev() {
                        if should_subscribe_thread(push_context, ev.raw()).await {
                            let Some(event_id) = ev.event_id() else {
                                // If the event doesn't have an event ID, we can't subscribe to it.
                                continue;
                            };
                            thread_subscriptions.0.insert(thread_root.clone(), event_id);
                            break;
                        }
                    }
                }
