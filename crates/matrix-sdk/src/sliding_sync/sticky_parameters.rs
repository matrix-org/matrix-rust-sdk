//! Sticky parameters are a way to spare bandwidth on the network, by sending request parameters once and have the server remember them.
//!
//! The set of sticky parameters have to be agreed upon by the server and the client; this is defined in the MSC.
...
pub trait StickyData {
    /// Request type that will be applied to, if the sticky parameters have been invalidated before.
    type Request;
...
///
/// Initially, the data provided is considered to be invalidated, so it's applied onto the request the first time it's sent.
/// Any changes to the wrapped data happen via `[Self::data_mut]`, which invalidates the sticky parameters; they will be applied automatically to the next request.
///
/// When applying sticky parameters, we will also remember the transaction id that was generated for us, stash it, so we can match the response against the transaction id later, and only consider the data isn't invalidated anymore (we say it's "committed" in that case) if the response's transaction id match what we expect.
#[derive(Debug)]
