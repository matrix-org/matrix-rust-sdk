...
- Matrix client (e.g. Riot) postMessage API (api: “fromWidget”) -- This API listens within
  a matrix client for inbound messages from widgets.
- Widget client postMessage API (api: “toWidget”) -- This API listens within
  a widget for inbound messages from matrix client, or the container embedding the widget.

The api follows a request/response pattern. So each request will be sent in both direction.
This makes for four valid postmessages structures:
...
- [`machine::Action`] describes an operation that the `WidgetDriver` wants to perform:
  ...
  - `MatrixDriverRequest`: A command sent from the client widget API state machine to the
    ...
    the client will typically generate an `Event` with the result.
...
```ascii
                                       Public API ──╮                                                      
                                                    ...
                                                    │ WidgetDriver                                     │   
┌────────────────┐  ToWidget response: -            │            ┌──────────────────────────────────┐  │   
│                │  FromWidget response: {...}      │            │WidgetMachine                     │  │   
│                ◄───────────────────╮              │            │                                  │  │   
│                │                   │              │            │                                  │  │   
│                │                 ┌─┴──────────────┴─────┐      │                                  │  │   
│                │    PostMessage  │                      │────╮ │                                  │  │   
