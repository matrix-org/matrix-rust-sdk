...
- `EventCache::save_events` has been removed. This API was ideal to create
  deadlocks in the Event Cache if not used with special care. Moreover, it
  breaks the isolation between components and was violating the clear separation
  of responsibility between components.
...
- Add `SlidingSync::set_room_subscriptions` for updating room subscriptions
  ...
  recreate all subscriptions. Instead, it removes existing subscriptions for
  rooms that are not resubscribed and adds new subscriptions for resubscribed
...
- The new `EventCache::forget_room` method properly forget a specific room. It
  ...
  deleted from the Event Cache entirely instead of being cleared only.
...
- Add `Client::rtc_transports` that queries the RTC transport advertised by the
  ...
  `Client::refresh_rtc_transports`, or to by-pass cache with
...
- `AttachmentConfig` gained an `extra_content` field and `RoomSendQueue::send()`
  ...
  vendor-prefixed keys that server-side push rules can match on (in lieu of
...
- Add `Client::disable_well_known_lookup`, which disables every
  ...
  Add `Client::discover_rtc_transports`, which returns the MatrixRTC transports
  advertised by the homeserver (MSC4519), with a fallback to the well-known
  `m.rtc_foci` as long as well-known lookup is allowed by the client. Transports
...
- `RoomEventCacheUpdate::AddEphemeralEvents` becomes
  `RoomEventCacheUpdate::AddReadReceipt`. It no longer contains an
...
- The Event Cache has now a unique state for all the caches for all the rooms.
  ...
  now reloaded at the same time. Previously, the invalidation was correctly
...
- A rare deadlock has been fixed in
  ...
  When aggregating the timeline for each thread, we are doing look ups in the
  room cache, and in the thread caches. Every time we do a look up with
  `find_event`, we are acquiring a read lock over the state. Because look up in
  the room cache can happen frequently, we were acquiring a read lock over the
  state for the room cache for the entire lifetime of the
  `aggregate_timeline_for_threads` function. However, in some situations, it
  ...
  up (L2). Before L2 is acquired, task T2 is running a different part of the
  code, like a pagination, and waits to acquire a write lock (L3). It waits
  because L1 is still alive and L3 needs an exclusive access. Now, L2 enters the
  game and cannot be acquired because L3 is waiting. So we have:
