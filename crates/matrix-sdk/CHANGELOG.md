...
- Update `Encryption::{spin_lock_store, try_lock_once_store}` so that lock dirtiness
  is determined entirely by `CrossProcessLock`, rather than logic defined by `OlmMachine`.
