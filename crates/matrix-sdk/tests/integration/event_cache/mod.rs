...
async fn test_backpaginate_empty_gap_reattaches_to_stored_events() {
    ...
    for _ in 0..5 {
        let outcome = room_event_cache.pagination().run_backwards_once(20).await.unwrap();
        recovered.extend(outcome.events.iter().filter_map(|e| e.event_id().map(ToOwned::to_owned)));
...
#[async_test]
async fn test_stale_sync_batch_does_not_reorder_the_tail() {
    let server = MatrixMockServer::new().await;
    let client = server.client_builder().build().await;
    ...
    // The room's tail is [$msg2, $msg3]. In the real-world incident this shape
    // comes from the send queue eagerly inserting our own just-sent events.
    server
        .sync_room(
            &client,
            JoinedRoomBuilder::new(room_id)
                .add_timeline_event(f.text_msg("two").event_id(event_id!("$msg2")))
                .add_timeline_event(f.text_msg("three").event_id(event_id!("$msg3"))),
        )
        .await;

    assert_let_timeout!(Ok(RoomEventCacheUpdate::UpdateTimelineEvents { .. }) = subscriber.recv());

    // Now a *stale* sync batch arrives: generated by the server before $msg3
    // existed, delivered late (think: a long-poll crawling over a bad network).
    // It carries $msg1, which we've never seen, and $msg2, which we know and
    // which is *not* our tail.
    server
        .sync_room(
            &client,
            JoinedRoomBuilder::new(room_id)
                .add_timeline_event(f.text_msg("one").event_id(event_id!("$msg1")))
                .add_timeline_event(f.text_msg("two").event_id(event_id!("$msg2"))),
        )
        .await;
    ...
    // $msg2 must not move past $msg3, and $msg1 must land before $msg2, where
    // the batch says it belongs: [$msg1, $msg2, $msg3]. The legacy
    // remove+re-append instead produced [$msg3, $msg1, $msg2] - permanently
    // reordered messages.
    let (events, _) = room_event_cache.subscribe().await.unwrap();
    assert_eq!(events.len(), 3);
    assert_event_id!(events[0], "$msg1");
    assert_event_id!(events[1], "$msg2");
    assert_event_id!(events[2], "$msg3");
}
...
#[async_test]
async fn test_gappy_stale_batch_does_not_strand_our_eager_tail() {
    // Deeper fix for the 2026-08-14 vanished-send incident: a stale
    // limited+gappy batch (generated before our just-sent tail event existed,
    // so strictly older than it) used to be appended AFTER our tail; the
    // shrink then hid our send behind the new gap until its sync echo
    // restored it. The eager tail suffix must instead be relocated past the
    // batch: the room stays [batch..., our send], no invisibility window.
    ...
    // Our own just-sent message is the tail, with a server timestamp newer
    // than anything the stale batch will carry.
    server
        .sync_room(
            &client,
            JoinedRoomBuilder::new(room_id).add_timeline_event(
                f.text_msg("just sent")
                    .sender(&own_user_id)
                    .server_ts(2000)
                    .event_id(event_id!("$sent")),
            ),
        )
        .await;
    ...
    // The stale limited+gappy batch: an unknown event, strictly older than
    // our send.
    server
        .sync_room(
            &client,
            JoinedRoomBuilder::new(room_id)
                ...
                .set_timeline_prev_batch("stale-token")
                .add_timeline_event(
                    f.text_msg("older").server_ts(1000).event_id(event_id!("$older")),
                ),
        )
        .await;
    ...
    // Our send must still be visible, at the tail, after the batch.
    let (events, _) = room_event_cache.subscribe().await.unwrap();
    assert_eq!(events.len(), 2);
    assert_event_id!(events[0], "$older");
    assert_event_id!(events[1], "$sent");

    // The late sync echo of our send dedups in place: no reorder, no
    // duplicate.
    server
        .sync_room(
            &client,
...
#[async_test]
async fn test_resolve_gap_anchors_across_redundant_gaps() {
    let server = MatrixMockServer::new().await;
    let client = server.client_builder().build().await;
    ...
    // Two limited syncs on top of some history:
    // `[$1, $2, $3] [gap "ga"] [$4] [gap "gb"] [$5]`.
    server
        .sync_room(
            &client,
            JoinedRoomBuilder::new(room_id)
    ...
    server
        .sync_room(
            ...
            JoinedRoomBuilder::new(room_id)
                .add_timeline_event(f.text_msg("four").event_id(event_id!("$4")))
                .set_timeline_prev_batch("ga".to_owned())
                .set_timeline_limited(),
        )
        .await;
    server
        .sync_room(
            &client,
            JoinedRoomBuilder::new(room_id)
                .add_timeline_event(f.text_msg("five").event_id(event_id!("$5")))
                .set_timeline_prev_batch("gb".to_owned())
                .set_timeline_limited(),
        )
        .await;
    ...
    assert_eq!(room_event_cache.timeline_gaps().await.unwrap().len(), 2);
    while room_stream.recv().now_or_never().is_some() {}

    // Resolving the newest gap walks all the way down into the oldest
    // chunk: everything between is accounted for, so the older gap is
    // redundant and goes away, and the one missed event lands in place.
    server
        .mock_room_messages()
        .match_from("gb")
        .ok(RoomMessagesResponseTemplate::default().end_token("g0").events(vec![
            f.text_msg("four").event_id(event_id!("$4")),
            f.text_msg("three").event_id(event_id!("$3")),
            f.text_msg("two and a half").event_id(event_id!("$2b")),
            f.text_msg("two").event_id(event_id!("$2")),
            f.text_msg("one").event_id(event_id!("$1")),
    ...
    assert!(room_event_cache.resolve_gap("gb".to_owned(), 20).await.unwrap());
    ...
    assert_matches!(&diffs[0], VectorDiff::Insert { index: 2, value: event } => {
        assert_event_matches_msg(event, "two and a half");
    });
