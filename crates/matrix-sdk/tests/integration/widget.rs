...
use ruma::{
    event_id,
    events::{room::member::MembershipState, AnyStateEvent, MessageLikeEventType, StateEventType},
    owned_room_id,
...
async fn test_receive_state() {
    ...
    let f = EventFactory::new().room(&ROOM_ID);
    let topic_event = f.room_topic("new room topic").sender(&BOB);
    let name_event_1 = f.room_name("new room name").sender(&BOB);
    let name_event_2 = f.room_name("even newer room name").sender(&BOB);
        ...
        .ok_and_run(&client, |sync_builder| {
            sync_builder.add_joined_room(
                JoinedRoomBuilder::new(&ROOM_ID)
                    ...
                    .add_timeline_event(f.text_msg("simple text message").sender(&ALICE))
                    // set room topic - doesn't match
                    .add_timeline_event(topic_event.clone())
                    .add_state_event(topic_event)
                    // set room name - matches filter but not reported in the state block
                    .add_timeline_event(name_event_1)
                    // set room name - matches filter
                    .add_timeline_event(name_event_2.clone())
                    .add_state_event(name_event_2.clone()),
            );
        })
        .await;
    ...
    assert_eq!(msg["api"], "toWidget");
    assert_eq!(msg["data"]["state"], json!([<Raw<AnyStateEvent>>::from(name_event_2)]));
