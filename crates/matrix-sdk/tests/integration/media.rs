...
async fn test_get_media_content_reports_download_progress() {
    ...
    let request_builder = media.get_media_content(&request, false);
    let subscriber = request_builder.subscribe_to_recv_progress();
...
async fn test_get_media_content_cache_hit_reports_full_progress() {
    ...
    let request_builder = media.get_media_content(&request, true);
    let subscriber = request_builder.subscribe_to_recv_progress();
    let content = request_builder.await.unwrap();
