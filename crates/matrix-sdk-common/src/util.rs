...
/// Platform agnostic helper function to create SecondsSinceUnixEpoch with modifications
pub fn modified_seconds_since_unix_epoch<F: Fn(Duration) -> Duration>(f: F) -> SecondsSinceUnixEpoch {
    let duration = f(
        SystemTime::now().duration_since(SystemTime::UNIX_EPOCH).expect("now is always higher")
    );
