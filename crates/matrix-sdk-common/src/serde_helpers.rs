...
///
/// The thread root is the field located at `content`.`m.relates_to`.`event_id`, if the field at
/// `content`.`m.relates_to`.`rel_type` is `m.thread`.
///
/// Returns `None` if we couldn't find a thread root, or if there was an issue during
/// deserialization.
...
mod tests {
    ...
    fn test_extract_bundled_thread_summary() {
        ...
        // When the bundled thread summary is malformed, we return `ThreadSummaryStatus::Unknown`.
        let event = Raw::new(&json!({
            "event_id": "$eid:example.com",
            "type": "m.room.message",
            "sender": "@alice:example.com",
