...
impl X509Signature {
    ...
    pub fn from_cms_pem(s: impl AsRef<str>) -> Result<Self, der::Error> {
        let wrapper = ContentInfoWrapper::from_pem(s.as_ref().as_bytes())?;
        Ok(Self(wrapper))
    }
...
impl Debug for X509Signature {
    fn fmt(&self, fmt: &mut Formatter<'_>) -> std::fmt::Result {
        fmt.debug_tuple("X509Signature").field(&self.to_cms_pem()).finish()
    }
}
