    ...
    () => {
        mod cryptostore_integration_tests {
            ...
            pub async fn get_loaded_store(name: &str) -> (ReadOnlyAccount, impl CryptoStore) {
                ...
                store.save_pending_changes(PendingChanges { account: Some(account.clone()), ..Default::default() }).await.expect("Can't save account");
            ...
            async fn save_account() {
                ...
                store.save_pending_changes(PendingChanges { account: Some(account), ..Default::default() }).await.expect("Can't save account");
                assert!(store.get_static_account().is_some());
            }
            ...
            async fn test_user_saving() {
                ...
                store.save_pending_changes(PendingChanges { account: Some(account.clone()), ..Default::default() })
                    .await
                    .expect("Can't save account");
