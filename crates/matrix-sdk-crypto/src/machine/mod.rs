...
impl OlmMachine {
    ...
    ///
    /// If the sender device is dehdrated, does no handling and immediately
    /// returns `Ok(None)`.
    ///
    ...
    async fn decrypt_to_device_event(
        ...
        let from_dehydrated_device = self
            .to_device_event_is_from_dehydrated_device(&decrypted, &event.sender)
            .await
            .map_err(|e| {
                error!(
                    error = ?e,
