...
#[async_test]
async fn test_excluding_insecure_means_custom_to_device_events_from_unverified_devices_are_ignored()
{
    ...
    assert_let!(
        ProcessedToDeviceEvent::UnverifiedSender { raw, encryption_info } = processed_event
    );

    // And the info provided in UnverifiedSender matches what we supplied
    let received: Value = serde_json::from_str(raw.json().get()).unwrap();
    assert_eq!(received.as_object().unwrap().get("content").unwrap(), &custom_content);
    assert_eq!(encryption_info.sender, tests::alice_id());
    assert_eq!(encryption_info.sender_device.as_ref().unwrap(), tests::alice_device_id());
    assert_eq!(
        encryption_info.verification_state,
        VerificationState::Unverified(VerificationLevel::UnsignedDevice)
    );
