...
pub fn timestamp_to_iso8601(ts: MilliSecondsSinceUnixEpoch) -> Option<String> {
    ...
    // has a max year of 285427, so `from_unix_timestamp_nanos` can overflow for
    // very large timestamps. (The Y10K problem!)
    let dt = OffsetDateTime::from_unix_timestamp_nanos(nanos_since_epoch).ok()?;

    // SAFETY: `format` can fail if:
    //
    // - The input lacks information on a component we have asked it to format (eg,
    //   it is given a `Time` and we ask it for a date), or
