...
impl UserIdentity {
    ...
    /// An identity mismatch is dectected when there is a trust problem with the
...
pub struct ReadOnlyUserIdentity {
    ...
    /// is the user wants.
    pinned_msk: Arc<Mutex<MasterPubkey>>,
}

/// Intermediate struct to help serialize ReadOnlyUserIdentity and support
/// versionning and migration.
/// Version v1 is adding support for pinning identitiez (`pinned_msk`), as part
        ...
        match value.version {
            None => {
                ...
                ReadOnlyUserIdentity {
                    ...
                    pinned_msk: Arc::new(Mutex::new(v0.master_key.clone())),
                }
            }
            _ => {
                // v1 format
                ...
                ReadOnlyUserIdentity {
                    ...
                    pinned_msk: Arc::new(Mutex::new(v1.pinned_msk)),
                }
            }
        }
    }
...
impl From<ReadOnlyUserIdentity> for ReadOnlyUserIdentitySerializer {
    fn from(value: ReadOnlyUserIdentity) -> Self {
        let v1 = ReadOnlyUserIdentityV1 {
            ...
            pinned_msk: value.pinned_msk.lock().unwrap().clone(),
        };
...
impl ReadOnlyUserIdentity {
    ...
    pub(crate) fn new(
        ...
        Ok(Self {
            ...
            pinned_msk: Mutex::new(master_key).into(),
        })
    }
    ...
    pub(crate) async fn from_private(identity: &crate::olm::PrivateCrossSigningIdentity) -> Self {
        ...
        Self {
            ...
            self_signing_key,
            pinned_msk: Arc::new(Mutex::new(master_key.clone())),
        }
    }
    ...
    pub(crate) fn pin(&self) {
        let mut m = self.pinned_msk.lock().unwrap();
    ...
    pub(crate) fn has_pin_violation(&self) -> bool {
        let pinned_msk = self.pinned_msk.lock().unwrap().clone();
    ...
    pub(crate) fn update(
        ...
        let new = Self {
            ...
            master_key: master_key.clone().into(),
            self_signing_key: self_signing_key.into(),
            pinned_msk: Mutex::new(pinned_msk).into(),
        };
...
pub(crate) mod tests {
    ...
    fn deserialization_migration_test() {
        ...
        let pinned_msk = migrated.clone().pinned_msk;
        let foo = pinned_msk.lock().unwrap();
        assert_eq!(foo.clone(), migrated.master_key.as_ref().clone());
    ...
    async fn resolve_identity_mismacth_with_verification() {
        ...
        let mut msk_to_udpate: CrossSigningKey =
            serde_json::from_value(DataSet::msk_b().get("@bob:localhost").unwrap().clone())
                .unwrap();

        msk_to_udpate.signatures.add_signature(
            my_user_id.to_owned(),
            usk_key_id.to_owned(),
