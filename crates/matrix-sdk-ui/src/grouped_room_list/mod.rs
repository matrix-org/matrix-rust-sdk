...
impl GroupedRoomListService {
    ...
    pub async fn setup(&self) {
        ...
        (*self.room_list_state.lock().await).room_list_update_handle =
    ...
    async fn update_rooms(
        ...
        for room in room_list_state.raw_rooms.iter() {
            ...
            // If it's a top level non-space room that's a direct descendant
            // keep it in place
            else if !room.is_space() && direct_descendants.contains(&room.room_id().to_owned()) {
                room_list.push(GroupedRoomListItem::Room(room.clone()));
            }
        ...
        let grouped_display_names = &room_list
            ...
            .map(|r| match r {
                GroupedRoomListItem::Space(space, room) => {
                    format!(
                        "(S:{:?}, R:{:?})",
                        space.name().or(Some(space.room_id().to_string())).unwrap(),
                        room.as_ref().map(|r| r.name().or(Some(r.room_id().to_string()))).unwrap()
                    )
                }
                GroupedRoomListItem::Room(room) => {
                    format!("R:{:?}", room.name().or(Some(room.room_id().to_string())).unwrap())
                }
            })
            .collect::<Vec<_>>();
