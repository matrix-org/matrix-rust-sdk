    ...
    F: Fn(&Room) -> (RoomState, Option<SuccessorRoomState>),
{
    fn matches(&self, room: &Room) -> bool {
        ...
        // Check if a room should be visible.
        match (room_state, successor_room_state) {
            // This room is joined, and there is no successor. It is the active version, and should
            // always be visible.
            (RoomState::Joined, None) => true,

            // This room is joined, and there is a successor room. This successor room is joined,
            // left or banned, so this room is **not** the active version, and should be hidden.
            ...
            // knocked, so this room **is** an active version, and should be visible.
            ...
            // not part of this room, so it should not be possible to know this room is tombstoned.
            // Consequently, this room **is** the active version, and should be visible.
            (RoomState::Left | RoomState::Invited | RoomState::Banned | RoomState::Knocked, _) => {
                true
            }
        }
    }
...
mod tests {
    ...
    #[async_test]
    async fn test_room_a_has_no_successor() {
    ...
    #[async_test]
    async fn test_room_a_has_a_joined_successor() {
        let (client, server) = logged_in_client_with_server().await;
        let [room] = new_rooms([room_id!("!a:b.c")], &client, &server).await;
