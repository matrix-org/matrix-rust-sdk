...
impl RoomListService {
    ...
    pub async fn subscribe_to_rooms(&self, room_ids: &[&RoomId]) {
        ...
        self.sliding_sync.resubscribe_to_rooms(
            ...
            self.must_cancel_in_flight_request(),
        )
    }

    /// Remove the subscriptions to `room_ids`; their latest events are kept.
    pub fn unsubscribe_to_rooms(&self, room_ids: &[&RoomId]) {
        self.sliding_sync.unsubscribe_to_rooms(room_ids, self.must_cancel_in_flight_request())
    }

    /// Subscribe to `room_ids` only, re-fetching the members of every room.
    pub async fn clear_and_subscribe_to_rooms(&self, room_ids: &[&RoomId]) {
        self.listen_to_latest_events(room_ids).await;
