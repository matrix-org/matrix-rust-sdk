...
mod tests {
    ...
    use ruma::{
        api::client::{
            ...
            sync::sync_events::v5::{Response, response},
        },
        assign, event_id,
        events::{
        ...
        owned_user_id, room_id, uint, user_id,
    };
    ...
    async fn test_notification_room_display_name_excludes_service_members() {
        ...
        let expected_display_name = "Alice, Bob".to_string();
        ...
        let event = f.text_msg("Hey").into_event();
        server
        ...
        let mut response_room = assign!(response::Room::new(), {
        ...
        server
            .mock_sliding_sync()
            .ok(assign!(Response::new("1".to_string()), {
