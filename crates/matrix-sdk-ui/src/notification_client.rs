                    ...
                    for _ in 0..3 {
                        ...
                        match new_event.kind {
                            matrix_sdk::deserialized_responses::TimelineEventKind::UnableToDecrypt (
                                utd_event) => {
                                if utd_event.utd_info.reason.is_missing_room_key() {
                                    // Decryption error that could be caused by a missing room
                                    // key; retry in a few.
                                    wait *= 2;
                                } else {
                                    trace!("Event could not be decrypted, but waiting longer is unlikely to help: {:?}", utd_event.utd_info.reason);
                                    return Ok(None);
                                }
                            }
                            _ => {
