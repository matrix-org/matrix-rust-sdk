...
impl NotificationClient {
    ...
    async fn try_sliding_sync(
        ...
        let ids_ref = Arc::new(
            requests
                .iter()
                .map(|req| (req.room_id.clone(), req.event_ids.clone()))
                .collect::<Vec<_>>(),
        );
        ...
        let timeline_event_handler = self.client.add_event_handler({
            ...
            move |raw: Raw<AnySyncTimelineEvent>| async move {
                match &raw.get_field::<OwnedEventId>("event_id") {
                    Ok(Some(event_id)) => {
                        let request =
                            &ids.iter().find(|(_, event_ids)| event_ids.contains(event_id));
                        if request.is_none() {
                            return;
                        }
                        let room_id = request.unwrap().0.clone();
                        for (_, event_ids) in ids.deref() {
    ...
    pub async fn get_notifications_with_sliding_sync(
        ...
        let mut ret = BatchNotificationFetchingResult::new();
        ...
        for (event_id, (room_id, raw_event)) in raw_events.into_iter() {
            ...
            if let Some(raw_event) = raw_event {
                let (raw_event, push_actions) = match &raw_event {
                    RawNotificationEvent::Timeline(timeline_event) => {
                        ...
                        match self.retry_decryption(&room, timeline_event).await {
                            ...
                            Ok(None) => {
                                match room.event_push_actions(timeline_event).await {
                                    ...
                                    Err(error) => {
                                        ...
                                        ret.mark_fetching_notification_failed(
                                            event_id,
                                            error.into(),
                                        );
                                        continue;
                            ...
                            Err(error) => {
                                ret.mark_fetching_notification_failed(event_id, error);
                                continue;
                            }
                        }
                    }
                    RawNotificationEvent::Invite(invite_event) => {
                        ...
                        match room.event_push_actions(invite_event).await {
                            ...
                            Err(error) => {
                                ret.mark_fetching_notification_failed(event_id, error.into());
                                continue;
                            }
                        }
                    }
                ...
                if !should_notify {
                    ret.add_notification(event_id, NotificationStatus::EventFilteredOut);
                } else {
                    ...
                    match notification_item {
                        Ok(notification_item) => {
                            ret.add_notification(event_id, notification_item);
                        }
                        Err(error) => {
                            ret.mark_fetching_notification_failed(event_id, error);
                        }
                ...
                ret.add_notification(event_id, NotificationStatus::EventNotFound);
            }
        }
