...
async fn test_update_read_marker() {
    ...
    // Nothing should happen, the marker cannot be added at the end. Timeline:
    // [A]. ^-- fully read
    ...
    // Now the read marker appears after the first event. Timeline:
    // [date-divider, A, read-marker, B]. fully read --^
    ...
    // at the end. Timeline: [date-divider, A, B]. ^-- fully read
    assert_next_matches!(stream, VectorDiff::Remove { index: 2 });

    // Timeline: [date-divider, A, B, C]. ^-- fully read
    timeline.handle_live_event(f.text_msg("C").sender(&BOB)).await;
    let item = assert_next_matches!(stream, VectorDiff::PushBack { value } => value);
    let event_id3 = item.as_event().unwrap().event_id().unwrap().to_owned();

    // Now the read marker is reinserted after the second event. Timeline:
    // [date-divider, A, B, read-marker, C]. ^-- fully read
    ...
    // Timeline: [date-divider, A, B, read-marker, C, D]. ^-- fully read
    ...
    // Timeline: [date-divider, A, B, C, D]. fully read --^
    assert_next_matches!(stream, VectorDiff::Remove { index: 3 });

    // Timeline: [date-divider, A, B, C, read-marker, D]. fully read --^
    ...
    // move. Timeline: [date-divider, A, B, C, read-marker, D, E]. fully read
    // --^
    ...
    // event sent from another user, then it will be removed. Timeline:
    // [date-divider, A, B, C, D, E]. fully read --^
    ...
    // Timeline: [date-divider, A, B, C, D, E, F]. fully read --^
    timeline.handle_live_event(f.text_msg("F").sender(&own_user)).await;
    let item = assert_next_matches!(stream, VectorDiff::PushBack { value } => value);
    item.as_event().unwrap();

    // Timeline: [date-divider, A, B, C, D, E, F, G]. fully read --^
    ...
    // just before their message. It is the first message that's both after the
    // fully-read event and not sent by us.
    //
    // Timeline: [date-divider, A, B, C, D, E, F, G, H]. fully read --^
    ...
    // ```
    //
    // Bob Timeline: [date-divider, A, B, C, D, E, F, G, read-marker, H]. fully
    // read --^
