...
use matrix_sdk::{
    attachment::{AttachmentConfig, GalleryConfig},
...
use super::{AttachmentSource, Error, GalleryItemInfo, Timeline};
...
impl<'a> IntoFuture for SendGallery<'a> {
    ...
    fn into_future(self) -> Self::IntoFuture {
        let Self { timeline, item_infos, config, tracing_span, use_send_queue, .. } = self;

        let fut = async move {
            let mapped_infos = item_infos
                .into_iter()
                .map(TryInto::try_into)
                .collect::<Result<Vec<matrix_sdk::attachment::GalleryItemInfo>, Error>>()?;

            if use_send_queue {
                let send_queue = timeline.room().send_queue();
                let fut = send_queue.send_gallery(mapped_infos, config);
                fut.await.map_err(|_| Error::FailedSendingAttachment)?;
            } else {
                todo!("implement queue-less sending for gallery events");
            }
