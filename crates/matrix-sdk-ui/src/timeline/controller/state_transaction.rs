...
use ruma::{
    ...
    events::{AnySyncTimelineEvent, receipt::ReceiptType},
    push::Action,
    serde::Raw,
};
...
impl<'a, P: RoomDataProvider> TimelineStateTransaction<'a, P> {
    ...
    pub(super) async fn handle_remote_event(
        ...
        let thread_summary = if let ThreadSummaryStatus::Some(ref summary) = event.thread_summary {
            ...
            let (own_thread_public_receipt, own_thread_private_receipt) = if let Some(ref latest_reply) = summary.latest_reply && 
                ...
                let own_thread_public_receipt = self
                    .meta
                    .read_receipts
                    .get_latest(&self.meta.own_user_id, &ReceiptType::Read)
                    .map(|(event_id, _receipt)| event_id.clone());

                let own_thread_private_receipt = self
                    .meta
                    .read_receipts
                    .get_latest(&self.meta.own_user_id, &ReceiptType::ReadPrivate)
                    .map(|(event_id, _receipt)| event_id.clone());
