...
impl Aggregations {
    ...
    fn promote_pending_beacon_stop(
        ...
        let stop_content = match &stop.kind {
            AggregationKind::BeaconStop { content } => content,
            _ => {
    ...
    pub fn apply_all(
        ...
        if let TimelineEventItemId::EventId(event_id) = item_id {
            if let Some(live_location) = event.content().as_live_location_state() {
                self.promote_pending_beacon_stop(
                    sender,
                    event_id.clone(),
                    &live_location.beacon_info,
                );
            }
        }
