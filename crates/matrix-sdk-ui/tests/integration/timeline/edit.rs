...
async fn test_send_edit() {
    ...
    let (_, mut timeline_stream) =
        timeline.subscribe_filter_map(|item| item.as_event().is_some().then_some(item)).await;
    ...
    let unique_id = hello_world_item.unique_id().clone();
    let hello_world_item = hello_world_item.as_event().unwrap();
    ...
    let edit_item = edit_item.as_event().unwrap();
...
async fn test_send_reply_edit() {
    ...
    let reply_unique_id = reply_item.unique_id().clone();

    let reply_item = reply_item.as_event().unwrap();
...
async fn test_edit_to_replied_updates_reply() {
    ...
    let replied_to_item = assert_next_matches!(timeline_stream, VectorDiff::PushBack { value } => {
        ...
        assert_eq!(event.content().as_message().unwrap().body(), "bonjour");
...
async fn test_send_edit_poll() {
    ...
    let poll_event_uid = poll_event.unique_id().clone();
    let poll_event = poll_event.as_event().unwrap();
...
async fn test_send_edit_when_timeline_is_clear() {
    ...
    // Start with a room that has a initial event.
    let f = EventFactory::new();
    ...
    // Observe it coming from the timeline instance.
    let hello_world_item =
        assert_next_matches!(timeline_stream, VectorDiff::PushBack { value } => value);
    let hello_world_unique_id = hello_world_item.unique_id().clone();
    let hello_world_item = hello_world_item.as_event().unwrap();
    let hello_world_message = hello_world_item.content().as_message().unwrap();
    assert!(!hello_world_message.is_edited());
    assert!(hello_world_item.is_editable());
    ...
    // The item is not edited, because it's disappeared from the timeline.
    assert!(!edited);
}
...
async fn test_edit_local_echo_with_unsupported_content() {
    ...
    let did_edit = timeline.edit(item.unique_id(), poll_start_content).await.unwrap();
