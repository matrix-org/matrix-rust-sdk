...
// Timing harness for `init_with_thread_root`, kept only as evidence for the
// concurrent related-events lookup; removed in the next commit.
#[async_test]
async fn test_zz_bench_thread_open_from_cache() {
    ...
    server
        .mock_room_event()
        .match_event_id()
        .ok(factory.text_msg("Thread root").event_id(&thread_root_event_id).into())
        .mount()
        .await;

    // 100 thread events, each with one reaction that only reaches the thread
    // timeline through the related-events lookup.
    let n = 100;
    let mut thread_events = Vec::new();
    let mut reactions = Vec::new();
    for i in 0..n {
        let event_id = ruma::OwnedEventId::try_from(format!("$t{i}")).unwrap();
        let previous = if i == 0 {
            thread_root_event_id.clone()
        } else {
            ruma::OwnedEventId::try_from(format!("$t{}", i - 1)).unwrap()
        };
        thread_events.push(
            factory
                .text_msg(format!("Threaded event {i}"))
                .event_id(&event_id)
                .in_thread(&thread_root_event_id, &previous)
                .into_raw(),
        );
        let reaction_id = ruma::OwnedEventId::try_from(format!("$r{i}")).unwrap();
        reactions.push(factory.reaction(&event_id, "👍").event_id(&reaction_id).into_raw_sync());
    }
    // /relations returns newest first.
    thread_events.reverse();

    server
        .mock_room_relations()
        .match_target_event(thread_root_event_id.clone())
        .ok(RoomRelationsResponseTemplate::default().events(thread_events))
        .mount()
        .await;

    let room = server.sync_joined_room(&client, room_id).await;

    // Fill the thread cache once. This timeline stays alive so the thread's
    // events stay in memory between the timed opens below.
    let first_timeline = TimelineBuilder::new(&room)
        .with_focus(TimelineFocus::Thread { root_event_id: thread_root_event_id.clone() })
        .build()
        .await
        .unwrap();
    assert!(first_timeline.paginate_backwards(u16::MAX).await.unwrap());

    server.sync_room(&client, JoinedRoomBuilder::new(room_id).add_timeline_bulk(reactions)).await;
    // The event cache persists sync updates from a background task.
    ...
    for _ in 0..10 {
        ...
        let items = timeline.items().await;
        let event_items = items.iter().filter(|item| item.as_event().is_some()).count();
        let with_reactions = items
            .iter()
            .filter(|item| {
                item.as_event().is_some_and(|event| {
                    event.content().reactions().is_some_and(|reactions| !reactions.is_empty())
                })
            })
            .count();
        // The root plus every thread event, each thread event with its reaction.
        assert_eq!(event_items, n + 1);
        assert_eq!(with_reactions, n);
    }

    samples.sort();
    eprintln!("BENCH median={:?} min={:?}", samples[samples.len() / 2], samples[0]);
}
