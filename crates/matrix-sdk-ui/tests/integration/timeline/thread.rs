...
async fn test_extract_bundled_thread_summary() {
    // A sync event that includes a bundled thread summary receives a `ThreadSummary` in the associated timeline content.
    let server = MatrixMockServer::new().await;
    let client = server.client_builder().build().await;
...
async fn test_new_thread_reply_causes_thread_summary() {
    ...
    //
    // TODO: maybe we should include the thread summaries if and only if the live timeline is
    // configured to exclude thread replies, aka, it requires thread-focused timelines to consult
    // the thread replies.
    assert_let_timeout!(Some(timeline_updates) = stream.next());
    ...
    // First, the replied-to event (thread root) doesn't have any thread summary info.
    ...
    // Since the replied-to item (the thread root) has been updated, all replies get updated too,
    // including the item we just pushed.
