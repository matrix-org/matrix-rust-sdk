...
async fn test_thread_backpagination() {
    ...
    // FIXME: add the thread root back!
    //assert_eq!(items[1].as_event().unwrap().content().as_message().unwrap().
    // body(), "Thread root");
...
async fn test_thread_filtering() {
    ...
    // event.
    {
        ...
        //assert_matches!(value.as_event().unwrap().content().thread_summary(),
        // Some(_));
