...
async fn test_redact_message() {
    ...
    let did_redact = timeline.redact(first.unique_id(), Some("inapprops")).await.unwrap();
    assert!(did_redact);
...
async fn test_redact_local_sent_message() {
    ...
    let item_uid = item.unique_id().clone();
    let event = item.as_event().unwrap();
    assert!(event.is_local_echo());
    ...
    timeline.redact(&item_uid, None).await.unwrap();
}
