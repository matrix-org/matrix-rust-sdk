...
#[derive(Debug, Error)]
pub enum IndexeddbMediaStoreTransactionError {
...
impl From<web_sys::DomException> for IndexeddbMediaStoreTransactionError {
    fn from(value: web_sys::DomException) -> Self {
        Self::DomException { name: value.name(), message: value.message(), code: value.code() }
    }
}

impl From<serde_wasm_bindgen::Error> for IndexeddbMediaStoreTransactionError {
    fn from(e: serde_wasm_bindgen::Error) -> Self {
        Self::Serialization(Box::new(serde_json::Error::custom(e.to_string())))
    }
}

impl From<IndexeddbMediaStoreTransactionError> for MediaStoreError {
    fn from(value: IndexeddbMediaStoreTransactionError) -> Self {
        use IndexeddbMediaStoreTransactionError::*;
...
impl<'a> IndexeddbMediaStoreTransaction<'a> {
    ...
    pub async fn commit(self) -> Result<(), IndexeddbMediaStoreTransactionError> {
    ...
    ) -> Result<Vec<T>, IndexeddbMediaStoreTransactionError>
    where
        T: Indexed,
        ...
        K: IndexedKey<T> + Serialize,
    {
        ...
        for value in array {
            let item = self
                .serializer
                .deserialize(value)
                .map_err(|e| IndexeddbMediaStoreTransactionError::Serialization(Box::new(e)))?;
            items.push(item);
        }
        Ok(items)
    }
    ...
    pub async fn get_item_by_key<T, K>(
        &self,
        key: K,
    ) -> Result<Option<T>, IndexeddbMediaStoreTransactionError>
    where
        T: Indexed,
        T::IndexedType: DeserializeOwned,
        T::Error: AsyncErrorDeps,
        K: IndexedKey<T> + Serialize,
    {
        ...
        if items.len() > 1 {
            return Err(IndexeddbMediaStoreTransactionError::ItemIsNotUnique);
        }
        Ok(items.pop())
    }
    ...
    pub async fn get_items_count_by_key<T, K>(
        &self,
        range: impl Into<IndexedKeyRange<K>>,
    ) -> Result<usize, IndexeddbMediaStoreTransactionError>
    where
        T: Indexed,
        T::IndexedType: DeserializeOwned,
        T::Error: AsyncErrorDeps,
    ...
    pub async fn add_item<T>(&self, item: &T) -> Result<(), IndexeddbMediaStoreTransactionError>
    where
        T: Indexed + Serialize,
    ...
    pub async fn put_item<T>(&self, item: &T) -> Result<(), IndexeddbMediaStoreTransactionError>
    where
        T: Indexed + Serialize,
        T::IndexedType: Serialize,
        T::Error: AsyncErrorDeps,
    ...
    pub async fn delete_items_by_key<T, K>(
        &self,
        range: impl Into<IndexedKeyRange<K>>,
    ) -> Result<(), IndexeddbMediaStoreTransactionError>
    where
        T: Indexed,
        K: IndexedKey<T> + Serialize,
    {
    ...
    pub async fn clear<T>(&self) -> Result<(), IndexeddbMediaStoreTransactionError>
    where
        T: Indexed,
    {
    ...
    pub async fn get_lease_by_id(
        &self,
        id: &str,
    ) -> Result<Option<Lease>, IndexeddbMediaStoreTransactionError> {
    ...
    pub async fn get_media_retention_policy(
        &self,
    ) -> Result<Option<MediaRetentionPolicy>, IndexeddbMediaStoreTransactionError> {
