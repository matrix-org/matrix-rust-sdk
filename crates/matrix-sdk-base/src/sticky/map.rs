...
/// recovers on its own.
pub(super) const MAX_ENTRIES: usize = 5000;
...
struct StickyValue {
    ...
    /// Identifies the insertion this value stems from, so that stale nodes in
    /// the expiry heap can be told apart from the current one.
    seq: u64,
}

impl StickyValue {
    /// The conflict-resolution rank of this value: higher wins.
    fn rank(&self) -> (u64, &OwnedEventId) {
        (self.order_ts, &self.event_id)
    }

    /// Whether this value is a live value at `now`, i.e. neither expired nor a
    /// tombstone.
...
pub(crate) struct EphemeralMap {
    entries: HashMap<StickyKey, StickyValue>,
...
impl EphemeralMap {
    ...
    fn insert(&mut self, now: u64, candidate: Candidate) -> Option<Change> {
        ...
        self.entries.insert(
            key.clone(),
            StickyValue { event_id, order_ts, expires_at, is_tombstone, kind, seq },
        );
    ...
    pub fn live(&self, now: u64) -> impl Iterator<Item = StickyEvent> + '_ {
        self.entries
            .iter()
            .filter(move |(_, value)| value.is_live(now))
            .map(|(key, value)| Self::event_from_entry(key, value))
    }
    ...
    pub fn evict_expired(&mut self, now: u64) -> Vec<StickyKey> {
        ...
        while let Some(Reverse(node)) = self.expiry_heap.peek() {
            ...
            let is_current = self.entries.get(&node.key).is_some_and(|value| value.seq == node.seq);

            if is_current
                && let Some(value) = self.entries.remove(&node.key)
                && !value.is_tombstone
            {
    ...
    pub fn clear(&mut self, now: u64) -> Vec<StickyKey> {
        ...
        self.entries.drain().filter(|(_, value)| value.is_live(now)).map(|(key, _)| key).collect()
    }

    fn event(&self, key: &StickyKey) -> Option<StickyEvent> {
        self.entries.get(key).map(|value| Self::event_from_entry(key, value))
    }

    fn event_from_entry(key: &StickyKey, value: &StickyValue) -> StickyEvent {
        StickyEvent {
            key: key.clone(),
            event_id: value.event_id.clone(),
            kind: value.kind.clone(),
            expires_at: MilliSecondsSinceUnixEpoch(UInt::new_saturating(value.expires_at)),
        }
    }
}
...
mod tests {
    ...
    fn test_new_keys_are_dropped_beyond_the_cap_but_updates_still_apply() {
        ...
        assert_let!([event] = update.updated.as_slice());
