...
pub trait EventCacheStore: AsyncTraitDeps {
    ...
    // TODO(bnjbvr): rename to `clear_all_linked_chunks`, since it's going to also
    // remove all the thread linked chunks later.
    ...
    /// Find an event by its ID in a linked chunk.
    async fn find_event(
        &self,
        lcid: LinkedChunkId<'_>,
        event_id: &EventId,
...
impl<T: EventCacheStore> EventCacheStore for EraseEventCacheStoreError<T> {
    ...
    async fn find_event(
        ...
        self.0.find_event(lcid, event_id).await.map_err(Into::into)
    }
    ...
    async fn find_event_relations(
        ...
        self.0.find_event_relations(lcid, event_id, filter).await.map_err(Into::into)
    }
