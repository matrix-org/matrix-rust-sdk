...
impl BaseClient {
    ...
    pub async fn clone_with_in_memory_state_store(
        ...
        let copy = Self {
            ...
            //    `OlmMachine`.
            // 2. We need to ensure that the parent and child use the same data
            //    and caches inside the `OlmMachine` so the various ratchets and
            //    places where new randomness gets introduced don't diverge,
            //    i.e. one-time keys that get generated by the Olm Account or
            //    Olm sessions when they encrypt or decrypt messages.
