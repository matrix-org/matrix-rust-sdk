...
impl StoreOpenConfig {
    /// Create a new [`StoreOpenConfig`] with a path representing the directory
    /// containing the store database, and its passphrase if encoded.
    pub fn new<P>(path: P, passphrase: Option<&str>) -> Self
    where
        P: AsRef<Path>,
    {
        Self {
...
mod tests {
    ...
    use super::StoreOpenConfig;

    #[test]
    fn test_store_open_config() {
        let store_open_config = StoreOpenConfig::new(Path::new("foo"), Some("bar"))
            .pool_max_size(42)
            .optimize(false)
            .cache_size(43)
