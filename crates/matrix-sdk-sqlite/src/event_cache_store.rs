...
async fn run_migrations(conn: &SqliteAsyncConn, version: u8) -> Result<()> {
    ...
    if version < 19 {
        ...
        conn.with_transaction(|txn| {
            txn.execute_batch(include_str!(
                "../migrations/event_cache_store/019_events_origin_server_ts.sql"
            ))?;
                ...
                match update {
                    ...
                    Update::PushItems { at, items } => {
                        ...
                        let mut content_statement = txn.prepare(
                            "INSERT OR REPLACE INTO events(room_id, event_id, event_type, session_id, content, relates_to, rel_type, origin_server_ts) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
                        )?;
                        ...
                        for (i, (event_id, event_type, event)) in items.into_iter().filter_map(invalid_event).enumerate() {
                            ...
                            // Table `events`.
                            {
                                ...
                                let origin_server_ts = event.timestamp.map(|ts| u64::from(ts.get()));
