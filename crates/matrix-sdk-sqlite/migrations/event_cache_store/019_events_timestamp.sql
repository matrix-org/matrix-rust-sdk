...
CREATE TABLE "events" (
    ...
    -- The timestamp of this event in milliseconds since Unix epoch, sourced
    -- from TimelineEvent::timestamp(). Used for MSC1763 retention policy
    -- enforcement. Null if the event has no known timestamp. Such events are
    -- left untouched by retention purging: SQL's `<` comparison against NULL
    -- is never true, so they are naturally excluded from
