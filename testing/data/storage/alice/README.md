...
# Note for WASM environment

The database file will be prefixed with `*.wasm.db` instead of `*.sqlite3`.
The provided `matrix-sdk-crypto.sqlite3` data format isn't compatible with SQLite compiled to WebAssembly.
The transcoded `matrix-sdk-crypto.wasm.db` was generated by dumping `matrix-sdk-crypto.sqlite3` 
and convert them to WASM compatible format via `https://sqlight.dev/` export tool.
