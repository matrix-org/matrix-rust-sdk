...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn test_notification_room_display_name_excludes_service_members() -> Result<()> {
    let alice = TestClientBuilder::new("alice").use_sqlite().build().await?;
    ...
    let charlie = TestClientBuilder::new("charlie").use_sqlite().build().await?;
    let charlie_user_id = charlie.user_id().unwrap().to_owned();

    // Initial setup: Alice creates a room, invites Bob and Charlie, sets Charlie as
    // a service member.
    let invite = vec![bob_user_id, charlie_user_id.clone()];
    let member_hints_content = MemberHintsEventContent::new(BTreeSet::from([charlie_user_id]));
    let initial_state =
        vec![InitialStateEvent::with_empty_state_key(member_hints_content).to_raw_any()];
    let request = assign!(CreateRoomRequest::new(), {
        invite,
        initial_state,
        is_direct: true,
    });
    let room_id = alice.create_room(request).await?.room_id().to_owned();

    // Bob and Charlie join room
    bob.sync_once(Default::default()).await?;
    bob.join_room_by_id(&room_id).await?;

    charlie.sync_once(Default::default()).await?;
    charlie.join_room_by_id(&room_id).await?;
    ...
    // In this sync, Alice receives message from Bob
    let response = alice.sync_once(SyncSettings::default()).await?;
    ...
    let event_id = response
        ...
        .expect("message from bob in alice's client");

    // The room name should simply be Bob's display name
    let expected_display_name = bob.account().get_display_name().await?.unwrap();

    let process_setup = NotificationProcessSetup::SingleProcess {
        sync_service: Arc::new(SyncService::builder(alice.clone()).build().await?),
    };

    // Check that the notification we get with `/context` shows the correct room
    // name
    let notification_client =
        NotificationClient::new(alice.clone(), process_setup.clone()).await.unwrap();
    assert_let!(
        NotificationStatus::Event(notification) =
            notification_client.get_notification_with_context(&room_id, &event_id).await?
    );
